//! Provider-agnostic AI settings: the stored key, and the connection test.
//!
//! Configuration calls work while AI is off; the network test is protected by
//! the backend opt-in gate.
...
use crate::ai::{
    prompts, AiConnectionResult, AiService, CONNECTION_TEST_TIMEOUT, CURATED_MODELS, DEFAULT_MODEL,
};
...
pub async fn get_ai_status(db: &Database, ai: &AiService) -> AppResult<AiStatus> {
...
pub async fn save_ai_key(ai: &AiService, api_key: String) -> AppResult<AiKeyStatus> {
    let credentials = ai.credentials.clone();
    run_credential_task(move || credentials.save(&api_key)).await?;
    Ok(AiKeyStatus { key_stored: true })
}

pub async fn remove_ai_key(ai: &AiService) -> AppResult<AiKeyStatus> {
    let credentials = ai.credentials.clone();
...
/// Credential stores block, so they run off the async runtime's worker
/// threads. The desktop store talks to the operating system; the server's
/// reads a file.
