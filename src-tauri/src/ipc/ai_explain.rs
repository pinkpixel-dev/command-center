...
pub async fn explain_command(
    ...
    let payload = serde_json::to_string(&generated).map_err(|err| {
        AppError::runtime(format!("could not store the explanation: {err}"))
    })?;
...
mod tests {
    use super::*;
    use crate::ai::explanation::{SafetyReview, PreviewCommand};
    ...
    /// The same gate the async command applies, without a Tauri state handle.
    fn read(db: &Database, command_id: i64) -> AppResult<Option<ExplanationView>> {
        db.with(|conn| {
            if !settings::load(conn)?.ai_enabled {
                return Ok(None);
            }
    ...
    fn a_cached_explanation_is_returned_whole_when_ai_is_on() {
        ...
        let loaded = read(&db, id).unwrap().expect("a cached explanation");
    ...
    fn cached_explanations_stay_hidden_while_ai_is_off() {
        ...
        assert_eq!(read(&db, id).unwrap(), None);
    ...
    fn clearing_works_even_after_ai_has_been_turned_off() {
        ...
        assert_eq!(db.with_mut(explanations::clear_all).unwrap(), 1);
    ...
    fn an_edited_entry_returns_its_explanation_marked_stale() {
        ...
        let loaded = read(&db, id).unwrap().unwrap();
        assert!(loaded.stale);
    ...
    fn an_entry_with_no_explanation_reports_nothing_rather_than_failing() {
        ...
        .unwrap();
        let id = db.with_mut(|conn| commands_db::create(conn, second)).unwrap().id;
